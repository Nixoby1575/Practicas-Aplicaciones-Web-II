// src/controllers/tipoExamenController.ts
import { Request, Response } from 'express';
import { PrismaClient } from '@prisma/client';
import { createTipoExamen } from '../factory/entityFactory';
...
const tipoExamenController = {
    ...
    create: async (req: Request, res: Response) => {
        ...
        try {
            const nuevoTipoExamen = createTipoExamen(descripcion, indicaciones);
            const tipoExamenCreado = await prisma.tipoExamen.create({
                data: nuevoTipoExamen,
            });
            res.json(tipoExamenCreado);
        } catch (error) {
