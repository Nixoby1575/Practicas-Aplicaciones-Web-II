...
let ResultadoService = class ResultadoService {
    ...
    async create(createResultadoDto) {
        const newResultado = this.resultadoRepository.create(createResultadoDto);
        return this.resultadoRepository.save(newResultado);
    }
    async findAllActive() {
        return this.resultadoRepository.find({ where: { estado: 'Activo' } });
    }
};
